...
def text_detail(request, course_pk, step_pk):
	...
	form = forms.TextForm(instance=step)

	if request.method == 'POST':
		form = forms.TextForm(instance=step, data=request.POST)
		if form.is_valid():
	...
	return render(request, 
		...
		{'step':step,'form':form, 'course':course})

@login_required	
def create_text(request, course_pk):
	course = get_object_or_404(models.Course, pk=course_pk)
	form = forms.TextForm()
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			text.course = course
			text.save()
			messages.success(request, "Text step added!")
